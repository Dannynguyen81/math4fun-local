...
export type SupabaseLeaderboardResult = {
  ok: boolean;
...
function compactProfileState(profile: StudentProfile): Json {
  const {
    ...
    role: _role,
    questionReports: _reports,
    goldHistory: _goldHistory,
    ...state
  } = profile;
  return asJson(state);
}
...
function getBadgeCount(profile: StudentProfile) {
  const elementBadges = Object.values(profile.elementXp).filter(
    xp => (xp ?? 0) >= 200
  ).length;
...
export function toLeaderboardEntry(profile: StudentProfile): LeaderboardEntry {
  ...
  const score =
    profile.xp +
    profile.completedStationIds.length * 250 +
    profile.collectedGuardianIds.length * 175 +
    badges * 200;
  return {
...
function guardianRows(profile: StudentProfile) {
  return profile.collectedGuardianIds.map(guardianId => ({
    ...
    training_xp: Math.max(
      0,
      Math.floor(profile.guardianTrainingXp[guardianId] ?? 0)
    ),
    health: Math.max(
      0,
      Math.min(100, Math.floor(profile.guardianHealth[guardianId]?.hp ?? 100))
    ),
...
export async function syncProfileToSupabase(
  ...
  if (profileWrite.error)
    return { ok: false, reason: profileWrite.error.message };
  ...
  if (profile.goldHistory.length)
    jobs.push(
      supabase.from("gold_ledger").upsert(
        profile.goldHistory.map(entry => ({
          id: entry.id,
          profile_id: profile.id,
          amount: entry.amount,
  ...
  if (profile.questionReports.length)
    jobs.push(
      supabase
        .from("reports")
        .upsert(profile.questionReports.map(reportRow), {
          onConflict: "id",
  ...
  if (guardians.length)
    jobs.push(
      supabase
        .from("guardian_collection")
        .upsert(guardians, { onConflict: "profile_id,guardian_id" })
    );
  const settled = await Promise.allSettled(jobs);
  const rejection = settled.find(result => result.status === "rejected");
  if (rejection?.status === "rejected")
    return { ok: false, reason: String(rejection.reason) };
  const writeError = settled.find(
    result =>
      result.status === "fulfilled" &&
      Boolean((result.value as SupabaseWriteResponse)?.error)
  );
  if (writeError?.status === "fulfilled") {
    const error = (writeError.value as SupabaseWriteResponse).error;
    return {
      ok: false,
      reason: error?.message ?? "Supabase từ chối ghi dữ liệu.",
    };
  }
  return { ok: true };
}
...
export async function fetchSupabaseLeaderboardResult(): Promise<SupabaseLeaderboardResult> {
  if (!supabase)
    return { ok: false, entries: [], reason: "Đồng bộ Supabase đang tắt." };
  const { data, error } = await supabase
    .from("leaderboard")
    .select(
      "profile_id, display_name, avatar, score, level, badges, guardians, stations, streak"
    )
  ...
  if (error || !data)
    return {
      ok: false,
      entries: [],
      reason: error?.message ?? "Không thể đọc bảng xếp hạng Supabase.",
    };
  return {
    ok: true,
    entries: (
...
export function cloudLedgerToLocal(entry: {
  id: string;
  amount: number;
  category: string;
  label: string;
  created_at: string;
}): GoldTransaction | null {
  if (
    !["answer", "hint", "station-open", "shop", "set-reward"].includes(
      entry.category
    )
  )
    return null;
  return {
    id: entry.id,
    amount: entry.amount,
    category: entry.category as GoldTransaction["category"],
    label: entry.label,
