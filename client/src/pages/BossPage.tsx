...
export default function BossPage() {
  const { profile, isBossUnlocked, startBattle, resolveBattleAnswer, advanceBattle, audioEnabled, guardianHp } = useGame();
  ...
  function begin() {
    if (startBattle(selectedGuardian.id)) {
  ...
  return (
    <section>
      ...
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">BOSS COMBAT · GUARDIAN SPELLS</p><h1 className="font-display text-4xl font-black tracking-tight">Atlas — Người giữ kho lưu trữ</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#58708b]">Chọn guardian để gọi đúng phép hệ của bạn ấy. Trả lời đúng vẫn nhận phản công 6–14 HP; sai mất 34 HP. Guardian về 0 HP sẽ rời Bộ sưu tập và phải thu phục lại qua trạm. Sinh lực tự hồi 20 HP mỗi giờ.</p></div><span className="field-tag"><Crown size={14} /> {status === "active" ? `Lượt ${(battle?.questionIndex ?? 0) + 1}/5` : `${unusedBossQuestions} câu H chưa dùng`}</span></div>
