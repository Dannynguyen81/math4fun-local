...
export default function BossPage({ mapId = 1 }: { mapId?: MapId }) {
  ...
  if (!bossUnlocked) {
    return <section className="overflow-hidden border-2 border-[#172a48] bg-[#172a48] text-white shadow-[6px_6px_0_#f6b73c]"><div className="relative h-52"><img src={ARENA_IMAGE} alt={`Đấu trường Boss Map ${mapId}`} className="h-full w-full object-cover opacity-55" /><div className="absolute inset-0 bg-gradient-to-t from-[#172a48] via-[#172a48]/50 to-transparent" /><img src={boss.sprite} alt={boss.name} className="absolute bottom-3 right-7 h-40 w-40 object-contain drop-shadow-[0_7px_0_rgba(0,0,0,.45)]" /><div className="absolute inset-x-5 bottom-5"><span className="field-tag border-white/30 bg-white/10 text-white"><Lock size={13} /> CỬA CUỐI MAP {mapId}</span><h1 className="mt-3 font-display text-4xl font-black">{boss.name} đang chờ</h1></div></div><div className="p-6"><p className="max-w-2xl text-sm leading-relaxed text-[#d5dfed]">Mở đủ 10 chủ đề của Map {mapId} để gọi {boss.name}. Trận có 10 câu trung bình và khó từ Tập {mapId}; chiến thắng nhận huy hiệu <b>{boss.badge}</b>{mapId === 1 ? " và mở Map 2." : "."}</p><Link href="/map" className="mt-5 inline-flex items-center gap-2 border-2 border-[#172a48] bg-[#f6b73c] px-4 py-3 font-bold text-[#172a48] shadow-[3px_3px_0_white]"><Swords size={16} /> Trở về tuyến đường</Link></div></section>;
  }
