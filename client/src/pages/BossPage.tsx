...
import { ARENA_IMAGE, BATTLE_AUDIO, MAP1_BOSS_QUESTION_IDS, MAP2_BOSS_QUESTION_IDS, MAP_BOSS_ARCHIVES, MAP_BOSS_RULES, getGuardian, getSpellForGuardian, GUARDIANS, MAGIC_MEDIA, QUESTIONS_BY_ID, STATIONS, type MapId } from "@/game/gameData";
import { type ElementLevelUp, useGame } from "@/contexts/GameContext";
import { playBossAttackSound, playBossResultSound, playElementLevelUpSound, playElementSound, playFiveStreakSound, playTechniqueSound } from "@/lib/magicAudio";
...
export default function BossPage({ mapId = 1 }: { mapId?: MapId }) {
  const { profile, isMap1BossUnlocked, isMap2BossUnlocked, isStationMastered, startMap1BossBattle, startMap2BossBattle, resolveBattleAnswer, advanceBattle, audioEnabled, guardianHp, reportQuestion, unlockQuestionHint } = useGame();
  ...
  const unusedBossQuestions = useMemo(() => bossPool.filter((id) => !bossHistory.includes(id)).length, [bossHistory, bossPool]);
  ...
  function begin() {
    const started = mapId === 1 ? startMap1BossBattle(selectedGuardian.id) : startMap2BossBattle(selectedGuardian.id);
    if (started) {
  ...
  function submit(answer: number) {
    ...
    window.setTimeout(() => playBossAttackSound(boss.element, audioEnabled), result.correct ? 270 : 150);
        ...
        <div className="boss-dossier-body">
          <div className="boss-route-evidence">
            ...
            {[
              ...
              ["03", `Gọi ${boss.name} ra đấu trường`, "Chọn guardian và dùng đúng phép hệ."],
          ...
          <aside className="boss-reward-specimen relative overflow-hidden">
            ...
            <div className="relative mx-auto mt-3 grid h-28 w-28 place-items-center rounded-full border-2 border-dashed border-[#172a48] bg-[#edf1e8] p-2 shadow-[2px_2px_0_#172a48]"><span aria-hidden="true" className="absolute -left-4 top-3 rotate-[-20deg] font-mono text-[8px] font-black tracking-[0.16em] text-[#58708b]">SPECIMEN 0{mapId}</span><img src={boss.sprite} alt={`Tiêu bản niêm phong của ${boss.name}`} className="h-20 w-20 object-contain grayscale-[0.35]" /><span className="absolute -bottom-2 border border-[#172a48] bg-[#f6b73c] px-2 py-0.5 font-mono text-[8px] font-black">BOSS · CHƯA GHI</span></div>
            <p className="mt-4 border-y border-dashed border-[#c9b88c] py-2 font-mono text-[9px] font-black tracking-[0.1em] text-[#58708b]">MẪU: {boss.name.toUpperCase()} · {boss.title.toUpperCase()}</p>
  ...
  if (!bossUnlocked) {
    const mapStations = STATIONS.filter((station) => station.book === (mapId === 1 ? "Tập 1" : "Tập 2"));
    const masteredMapStations = mapStations.filter((station) => isStationMastered(station.id)).length;
    return <section className="boss-unopened-dossier"><header className="boss-dossier-cover"><div><span className="field-tag border-white/30 bg-white/10 text-white"><ShieldAlert size={13} /> CỬA CUỐI MAP {mapId} · HỒ SƠ NIÊM PHONG</span><h1>Hồ sơ đấu trường {boss.name}</h1><p>{boss.name} chỉ xuất hiện khi mười mốc của Map {mapId} đã được đóng dấu. Đấu Boss là một case file tổng hợp, không phải một banner rời tuyến hành trình.</p></div><div className="boss-seal-stamp"><Lock size={20} /><b>KHÓA</b><small>{masteredMapStations}/10 MỐC</small></div></header><div className="boss-dossier-body"><div className="boss-route-evidence"><p className="section-kicker">DÂY MỞ NIÊM PHONG</p>{[["01", "Mở đủ 10 trạm", `${masteredMapStations}/10 mốc Map ${mapId} đã làm chủ.`], ["02", "Chuẩn bị đội guardian", `${profile.teamGuardianIds.length}/3 companion đang được ghi vào battle permit.`], ["03", `Đấu ${boss.name}`, "10 câu M/H không lặp · Boss phản công ở mọi lượt."]].map(([step, title, note], index) => <div className="boss-evidence-step" key={step}><b>{step}</b><span aria-hidden="true" /><div><strong>{title}</strong><small>{note}</small></div>{index < 2 && <i aria-hidden="true" />}</div>)}</div><aside className="boss-reward-specimen"><span className="field-tag"><Crown size={13} /> DẤU THƯỞNG CỬA CUỐI</span><div className="relative mx-auto mt-3 grid h-28 w-28 place-items-center rounded-full border-2 border-dashed border-[#172a48] bg-[#edf1e8] p-2 shadow-[2px_2px_0_#172a48]"><span aria-hidden="true" className="absolute -left-4 top-3 rotate-[-20deg] font-mono text-[8px] font-black tracking-[0.16em] text-[#58708b]">SPECIMEN 0{mapId}</span><img src={boss.sprite} alt={`Tiêu bản niêm phong của ${boss.name}`} className="h-20 w-20 object-contain grayscale-[0.35]" /><span className="absolute -bottom-2 border border-[#172a48] bg-[#f6b73c] px-2 py-0.5 font-mono text-[8px] font-black">BOSS · CHƯA GHI</span></div><p className="mt-4 border-y border-dashed border-[#c9b88c] py-2 font-mono text-[9px] font-black tracking-[0.1em] text-[#58708b]">MẪU: {boss.name.toUpperCase()} · {boss.title.toUpperCase()}</p><b>{boss.badge}</b><p>{mapId === 1 ? "Chiến thắng ghi huy hiệu và mở Map 2." : "Chiến thắng đóng dấu hoàn tất hành trình Toán 4."}</p></aside></div><footer className="boss-dossier-footer"><span>Điểm xuất phát được đánh dấu bằng la bàn Marigold và dây chứng cứ Map {mapId}.</span><Link href="/map" className="boss-dossier-cta"><Swords size={16} /> Trở về tuyến đường</Link></footer></section>;
  }
  ...
  const battleGuardian = getGuardian(battle?.guardianId ?? selectedGuardian.id) ?? selectedGuardian;

  return (
    <section>
      <audio ref={audioRef} src={BATTLE_AUDIO} loop preload="auto" />
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">MAP {mapId} BOSS · GUARDIAN SPELLS</p><h1 className="font-display text-4xl font-black tracking-tight">Boss tổng hợp — {boss.title}</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#58708b]">Mười câu trung bình và khó được rút từ các trạm Tập {mapId}. Trả lời đúng vẫn nhận phản công; sai mất nhiều HP hơn. Chiến thắng ghi huy hiệu <b>{boss.badge}</b>{mapId === 1 ? " và mở toàn bộ tuyến Tập 2." : "."}</p></div><span className="field-tag"><Crown size={14} /> {status === "active" ? `Lượt ${(battle?.questionIndex ?? 0) + 1}/10` : `${unusedBossQuestions} câu M/H chưa dùng`}</span></div>
      ...
      {status !== "active" ? (
        ...
        <div className="overflow-hidden border-2 border-[#172a48] bg-[#fffdf6] shadow-[6px_6px_0_#172a48]">
          <div className="relative min-h-[310px] overflow-hidden bg-[#172a48] p-5 sm:min-h-[360px]"><img src={ARENA_IMAGE} alt={`Đấu trường Boss ${boss.name}`} className="absolute inset-0 h-full w-full object-cover opacity-70" /><div className="absolute inset-0 bg-gradient-to-r from-[#172a48]/90 via-[#172a48]/35 to-[#172a48]/90" />
            ...
            <AnimatePresence>{feedback?.playerDamage ? <motion.div aria-hidden className="pointer-events-none absolute inset-0 z-20 border-4 border-[#ee6b4e] bg-[#ee6b4e]/15 mix-blend-screen" initial={{ opacity: 0, scale: 1 }} animate={reducedMotion ? { opacity: [0, 0.48, 0] } : { opacity: [0, 0.72, 0.12, 0], scale: [1, 1.018, 1] }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0.24 : 0.42, ease: [0.23, 1, 0.32, 1] }} /> : null}</AnimatePresence>
            ...
            <div className="relative grid gap-6 sm:grid-cols-2"><motion.div animate={feedback?.playerDamage && !reducedMotion ? { x: [0, -8, 8, -5, 3, 0] } : { x: 0 }} transition={{ duration: 0.34 }} className="pt-3"><p className="font-mono text-[10px] font-bold tracking-[0.16em] text-[#f6b73c]">{battleGuardian.name.toUpperCase()} · HP {playerHp}/100</p><div className="mt-2 h-3 overflow-hidden border border-white/50 bg-[#172a48]"><motion.div animate={{ width: `${playerHp}%` }} className="h-full bg-[#3e9b7a]" /></div><p className="mt-1 text-[10px] text-[#d5dfed]">Sai: −{MAP_BOSS_RULES[mapId].wrongAnswerDamage} HP · Đúng: vẫn chịu phản công phép.</p><div className="mt-4 flex flex-wrap gap-3">{team.map((guardianId) => <button key={guardianId} disabled className="cursor-default text-left disabled:opacity-60"><GuardianCaster guardianId={guardianId} active={guardianId === battleGuardian.id} isCasting={Boolean(feedback) && guardianId === battleGuardian.id} /></button>)}</div></motion.div><motion.div animate={feedback?.bossDamage && !reducedMotion ? { x: [0, 10, -8, 0] } : { x: 0 }} transition={{ duration: 0.3 }} className="sm:text-right"><p className="font-mono text-[10px] font-bold tracking-[0.16em] text-[#f6b73c]">BOSS {boss.name.toUpperCase()} · HP {bossHp}/{bossMaxHp}</p><div className="mt-2 h-3 overflow-hidden border border-white/50 bg-[#172a48]"><motion.div animate={{ width: `${(bossHp / bossMaxHp) * 100}%` }} className="h-full bg-[#ee6b4e]" /></div><motion.img animate={{ y: reducedMotion ? 0 : [-3, 3, -3], rotate: feedback?.bossDamage && !reducedMotion ? [0, 4, -4, 0] : 0 }} transition={{ duration: feedback?.bossDamage ? 0.3 : 2.2, repeat: feedback?.bossDamage || reducedMotion ? 0 : Infinity }} src={boss.sprite} alt={boss.name} className="ml-auto mt-3 h-36 w-36 object-contain drop-shadow-[0_6px_0_rgba(23,42,72,.65)] sm:h-44 sm:w-44" /><div className="mt-1 inline-flex items-center gap-2 border-2 border-white/60 bg-[#172a48]/80 px-3 py-1.5 font-display text-xl font-black text-white"><Crown className="text-[#f6b73c]" /> {boss.name}</div></motion.div></div>
          </div>
          <div className="border-t-2 border-[#172a48] bg-[#fffdf6] p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-2"><p className="section-kicker">Boss Map {mapId} · pool {bossPool.length} câu M/H</p><span className="field-tag"><Volume2 size={14} /> {audioEnabled ? "battle bật" : "battle tắt"}</span></div><div className="mt-4"><p className="font-display text-2xl font-black leading-tight sm:text-3xl">{question?.prompt}</p><div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-[#58708b]"><span>Nguồn: {question?.source}</span><button onClick={reportCurrentQuestion} className="inline-flex items-center gap-1 border-b border-dashed border-[#a54539] pb-0.5 font-bold text-[#a54539]"><Flag size={13} /> Báo lỗi</button></div></div><div className="mt-4 spell-ready-card" style={spellStyle(selectedGuardian.element)}><img src={selectedGuardian.sprite} alt={selectedGuardian.name} /><div><span>{selectedGuardian.type}</span><b>{selectedGuardian.name} · {spell.name}</b><small>{spell.note} +{spell.damage} / phản công −{spell.counterDamage}</small></div></div>
