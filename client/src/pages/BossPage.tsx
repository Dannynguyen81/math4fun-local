...
import { ARENA_IMAGE, BATTLE_AUDIO, MAP1_BOSS_QUESTION_IDS, getGuardian, getSpellForGuardian, GUARDIANS, MAGIC_MEDIA, QUESTIONS_BY_ID } from "@/game/gameData";
...
export default function BossPage() {
  const { profile, isMap1BossUnlocked, startMap1BossBattle, resolveBattleAnswer, advanceBattle, audioEnabled, guardianHp } = useGame();
  ...
  const unusedBossQuestions = useMemo(
    () => (profile ? MAP1_BOSS_QUESTION_IDS.filter((id) => !profile.map1BossQuestionHistory.includes(id)).length : 0),
    [profile],
  );
  ...
  function begin() {
    if (startMap1BossBattle(selectedGuardian.id)) {
  ...
  if (!isMap1BossUnlocked) {
    return <section className="overflow-hidden border-2 border-[#172a48] bg-[#172a48] text-white shadow-[6px_6px_0_#f6b73c]"><div className="relative h-52"><img src={ARENA_IMAGE} alt="Đấu trường Boss Map 1" className="h-full w-full object-cover opacity-55" /><div className="absolute inset-0 bg-gradient-to-t from-[#172a48] via-[#172a48]/50 to-transparent" /><div className="absolute inset-x-5 bottom-5"><span className="field-tag border-white/30 bg-white/10 text-white"><Lock size={13} /> CỬA CUỐI MAP 1</span><h1 className="mt-3 font-display text-4xl font-black">Boss tổng hợp đang chờ</h1></div></div><div className="p-6"><p className="max-w-2xl text-sm leading-relaxed text-[#d5dfed]">Hoàn tất các trạm Map 1 đã mở để gọi Boss tổng hợp. Trận này có 10 câu trung bình và khó từ hành trình Tập 1; chiến thắng sẽ mở Map 2.</p><Link href="/map" className="mt-5 inline-flex items-center gap-2 border-2 border-[#172a48] bg-[#f6b73c] px-4 py-3 font-bold text-[#172a48] shadow-[3px_3px_0_white]"><Swords size={16} /> Trở về tuyến đường</Link></div></section>;
  }

  const playerHp = battle?.playerHp ?? 100;
  const bossHp = battle?.bossHp ?? 150;
  const hasFreshRun = unusedBossQuestions >= 10 || profile.map1BossDefeated;
  ...
  return (
    <section>
      ...
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">MAP 1 BOSS · GUARDIAN SPELLS</p><h1 className="font-display text-4xl font-black tracking-tight">Boss tổng hợp — Cửa La Bàn Vàng</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#58708b]">Mười câu trung bình và khó được rút từ các trạm Tập 1. Trả lời đúng vẫn nhận phản công; sai mất nhiều HP hơn. Chiến thắng ghi dấu Map 1 và mở toàn bộ tuyến Tập 2.</p></div><span className="field-tag"><Crown size={14} /> {status === "active" ? `Lượt ${(battle?.questionIndex ?? 0) + 1}/10` : `${unusedBossQuestions} câu M/H chưa dùng`}</span></div>

      {status !== "active" ? (
            <article className="arena-dossier overflow-hidden border-2 border-[#172a48] bg-[#172a48] text-white shadow-[6px_6px_0_#f6b73c]"><div className="relative h-56"><img src={ARENA_IMAGE} alt="Đấu trường ma thuật Atlas" className="h-full w-full object-cover opacity-70" /><div className="absolute inset-0 bg-gradient-to-t from-[#172a48] via-[#172a48]/60 to-transparent" /><div className="absolute inset-x-5 bottom-5"><span className="field-tag border-white/25 bg-white/10 text-white">{hasFreshRun ? <ShieldAlert size={13} className="text-[#f6b73c]" /> : <Lock size={13} className="text-[#f6b73c]" />} {hasFreshRun ? "10 CÂU M/H · KHÔNG LẶP" : "ARCHIVE ĐÃ HẾT CÂU MỚI"}</span><h2 className="mt-3 font-display text-4xl font-black">{status === "victory" ? "Atlas đã trao dấu ấn cuối." : status === "defeat" ? `${battleGuardian.name} đã gục ngã và trở về trạm.` : "Chọn guardian rồi bước vào."}</h2></div></div><div className="grid gap-5 p-6 lg:grid-cols-[1fr_auto]"><p className="max-w-2xl text-sm leading-relaxed text-[#d5dfed]">{!hasFreshRun ? `Atlas còn ${unusedBossQuestions} câu M/H chưa dùng — chưa đủ mười câu cho một trận mới.` : status === "defeat" ? "Guardian đã rời Bộ sưu tập và dấu trạm tương ứng đã được mở lại. Hoàn thành lại 10 câu đúng để thu phục bạn ấy lần nữa." : `${selectedGuardian.name} đang có ${guardianHp(selectedGuardian.id)}/100 HP. Mỗi guardian có một phép riêng theo nguyên tố.`}</p><button onClick={begin} disabled={!hasFreshRun || !selectedGuardianId} className="inline-flex h-fit items-center gap-2 border-2 border-[#172a48] bg-[#f6b73c] px-5 py-3 font-bold text-[#172a48] shadow-[3px_3px_0_white] disabled:cursor-not-allowed disabled:opacity-45"><Swords size={17} /> {status === "idle" ? "Bắt đầu trận 10 câu" : "Thách đấu lại"}</button></div></article>
      ) : (
        <div className="overflow-hidden border-2 border-[#172a48] bg-[#fffdf6] shadow-[6px_6px_0_#172a48]">
          ...
          <div className="p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-2"><p className="section-kicker">Boss Map 1 · pool {MAP1_BOSS_QUESTION_IDS.length} câu M/H</p><span className="field-tag"><Volume2 size={14} /> {audioEnabled ? "nhạc battle bật" : "nhạc battle tắt"}</span></div><div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto]"><div><p className="font-display text-2xl font-black leading-tight sm:text-3xl">{question?.prompt}</p><p className="mt-2 text-xs font-semibold text-[#58708b]">Nguồn: {question?.source}</p></div><div className="spell-ready-card" style={spellStyle(selectedGuardian.element)}><img src={selectedGuardian.sprite} alt={selectedGuardian.name} /><div><span>{selectedGuardian.type}</span><b>{selectedGuardian.name} · {spell.name}</b><small>{spell.note} +{spell.damage} / phản công −{spell.counterDamage}</small></div></div></div>
