...
function RoutePreview({ signed }: { signed: boolean }) {
  return <article className="relative mt-6 overflow-hidden border-2 border-[#172a48] bg-[#fffdf6] p-5 shadow-[5px_5px_0_#172a48]">
    <div className="absolute right-4 top-3 rotate-6 border-2 border-dashed border-[#4d8b67] px-2 py-1 font-mono text-[9px] font-black tracking-[.16em] text-[#4d8b67]">ROUTE SPECIMEN</div>
    ...
    <div className="relative mt-5 grid gap-3 md:grid-cols-4">
      <div className="absolute left-[11%] right-[11%] top-7 hidden border-t-2 border-dashed border-[#4d8b67] md:block" />
      {STATIONS.slice(0, 4).map((station, index) => <div key={station.id} className={`relative ${index % 2 ? "md:translate-y-5" : ""}`}>
        <span className={`mx-auto grid h-14 w-14 place-items-center rounded-full border-2 border-[#172a48] font-display text-xl font-black shadow-[2px_2px_0_#172a48] ${signed ? "bg-[#f6b73c]" : "bg-[#eef1fb]"}`}>{index + 1}</span>
        <div className="mt-2 border-2 border-[#172a48] bg-white p-2 text-center shadow-[2px_2px_0_#172a48]"><b className="font-mono text-[9px] tracking-[.12em]">{station.code}</b><p className="mt-1 text-xs font-bold leading-tight">{station.title}</p></div>
      </div>)}
    </div>
